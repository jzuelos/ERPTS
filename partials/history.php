        ...
        <table class="table align-middle mb-0">
          ...
          <tbody>
            <?php foreach ($history_records as $record): 
              ...
              if ($p_info) {
                  $location_parts = [];
                  if (!empty($p_info['street'])) $location_parts[] = $p_info['street'];
                  if (!empty($p_info['barangay'])) $location_parts[] = $p_info['barangay'];
                  if (!empty($p_info['city'])) $location_parts[] = $p_info['city'];
                  if (!empty($p_info['province'])) $location_parts[] = $p_info['province'];
...
<div class="modal fade" id="historicalSnapshotModal" tabindex="-1" aria-labelledby="historicalSnapshotModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title" id="historicalSnapshotModalLabel">
          <i class="bi bi-clock-history me-2"></i>Historical Property Snapshot
        </h5>
...
function displaySnapshot(snapshot, createdAt) {
  ...
  let html = `
    <div class="alert alert-info mb-4">
      <i class="bi bi-info-circle me-2"></i>
      <strong>Snapshot Date:</strong> ${new Date(createdAt).toLocaleString('en-US', {
