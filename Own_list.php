...
// Handle AJAX request for barangays + district
if (isset($_GET['fetch']) && $_GET['fetch'] === 'location' && isset($_GET['m_id'])) {
  require_once 'database.php';
  $conn = Database::getInstance();
  $m_id = intval($_GET['m_id']);

  // Fetch district for the municipality
  $district_stmt = $conn->prepare("SELECT district_id, description FROM district WHERE m_id = ?");
  $district_stmt->bind_param("i", $m_id);
  ...
  $barangays_stmt = $conn->prepare("SELECT brgy_id, brgy_name FROM brgy WHERE m_id = ?");
  $barangays_stmt->bind_param("i", $m_id);
...
<body>
            ...
            <?php if (!empty($owners)): ?>
              <?php foreach ($owners as $owner): ?>
                <tr>
                  ...
                  <td><?= htmlspecialchars($owner['own_fname'] . ' ' . $owner['own_mname'] . ' ' . $owner['own_surname']) ?>
                  </td>
                  <td>
                    <?= htmlspecialchars($owner['house_no'] . ', ' . $owner['street'] . ', ' . $owner['barangay'] . ', ' . $owner['city'] . ', ' . $owner['district'] . ', ' . $owner['province']) ?>
                  </td>
            ...
            <div class="row g-3">
              ...
              <div class="col-md-4">
                <label for="barangay" class="form-label">
                  <span class="text-danger">*</span> Barangay
                </label>
                <select class="form-select" id="barangay" name="barangay" required>
                  <option value="" disabled selected>Select Barangay</option>
                  <?php foreach ($barangays as $brgy): ?>
                    <option value="<?= htmlspecialchars($brgy['brgy_id']) ?>">
              ...
              <div class="col-md-4">
                ...
                <select class="form-select" id="district" name="district" required>
                  ...
                  <?php foreach ($districts as $dist): ?>
                    <option value="<?= htmlspecialchars($dist['district_id']) ?>">
                      <?= htmlspecialchars($dist['description']) ?>
                    </option>
                  <?php endforeach; ?>
                </select>
              </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="input-group mb-3" style="max-width: 300px;">
            <input type="text" class="form-control" id="modalSearchInput" placeholder="Search by Name or Address"
                ...
                <?php if (!empty($owners)): ?>
                  <?php foreach ($owners as $owner): ?>
                    <tr>
                      <td><?= htmlspecialchars($owner['own_id']) ?></td>
                      <td>
                        <?= htmlspecialchars($owner['own_fname'] . ' ' . $owner['own_mname'] . ' ' . $owner['own_surname']) ?>
                      </td>
                      <td>
                        <?= htmlspecialchars($owner['house_no'] . ', ' . $owner['street'] . ', ' . $owner['barangay'] . ', ' . $owner['city'] . ', ' . $owner['district'] . ', ' . $owner['province']) ?>
                      </td>
  ...
  <script>
    // When Municipality/City changes
    $('#city').on('change', function () {
      const m_id = $(this).val();
      if (!m_id) return;

      $.ajax({
        url: window.location.pathname, // same PHP file
        type: 'GET',
        data: { fetch: 'location', m_id },
        success: function (response) {
          try {
            ...
            if (data.district) {
              districtSelect.append(
                `<option value="${data.district.district_id}" selected>${data.district.description}</option>`
              );
            } else {
              districtSelect.append('<option value="" disabled selected>No district available</option>');
            }
            ...
            if (data.barangays && data.barangays.length > 0) {
              ...
              barangaySelect.append('<option value="" disabled selected>No barangays available</option>');
            }
